...
nerd_red %>% 
  ggplot(aes(x=age)) +
  geom_histogram(fill = "#dca3a3", colour = "black") # colour is sometimes the border or the inside colour
```
...
nerd_red %>% 
  ggplot(aes(x = gender, y = age, colour = gender)) +
  geom_boxplot()

nerd_red %>% 
  ggplot(aes(x = gender, y = age, fill = gender)) + # fill = inside colour gets changed
  geom_boxplot() +
...
nerd_red %>% 
  ...
  geom_boxplot() +
  labs(title = "nerd score by gender") +
...
```{r}
nerd_red 
# all items for the test are in one column 

...
t.test(nerd_fm$nerd_score ~ nerd_fm$gender) # I want the nerd_score grouped by gender 
# default in R is Welch t-test b/c it does not assume equal variance (Varianzhomogenität)
```
...
```{r}
# run the same t-test for independent groups with var.equal = TRUE
t.test(nerd_fm$nerd_score ~ nerd_fm$gender, var.equal = TRUE) # the nerd_score as the dependent variable, testing between the two groups (gender)
# one single column that was grouped with another column
```
...
t.test(nerd_fm$Q1, nerd_fm$Q2, paired = TRUE) # now we do not have a grouping variable
```
...
# use on variable in one column that you can use as a grouping variable
```
...
```{r}
ttestgender <- t.test(nerd_fm$nerd_score ~ nerd_fm$gender, var.equal = TRUE)
ttestgender$p.value

t.test(nerd_fm$nerd_score ~ nerd_fm$gender, var.equal = TRUE)$p.value
```

2.2) This allows you to print your results right in your R Markdown document, e.g.: The p-value was `round(ttest_gender$p.value, 3)`.
...
3.2) The function `cohen_d()` from the package follows exactly the same logic as the `t.test()` function. Can you run an independent and a paired t-test, and then calculate the corresponding effect size? `round(cohens_d(nerd_fm$nerd_score ~ nerd_fm$gender)$Cohens_d, 3)`
```{r}
round(cohens_d(nerd_fm$nerd_score ~ nerd_fm$gender)$Cohens_d, 3)
```

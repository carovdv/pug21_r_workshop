...
```{r}
vector_char <- c("Koeln", "Berlin", "Hamburg", "München")
vector_char

vector_num <- c(2:5, 13)
vector_num

vector_num2 <-c(3:6, 14)
vector_num2

vector_dec <- c(1.32, 1.36, 1.39, 2.42)
vector_dec

vector_log <- c(FALSE, TRUE, TRUE, FALSE)
vector_log
```
...
vector1 <-  c(1, 5, 9)
vector2 <-  c(1, 9)
c(vector1, vector2, 5, 12, 30)

vector3 <-  c(1, 24, 353)
vector4 <-  c("Kaffee", "Kuchen")
c(vector3, vector4)
# man kann keine Daten mischen, wenn ich numerische mit character mische, dann werden darauf characters

c(vector_num + vector_num2)
c(vector_num, vector_num2, vector_log, vector_char)
c(vector_char, vector_dec)
```
...
What happens here?
numbers are added element wise 
...
1.5) `c(2, 4, 6) - c(1, 0, 1)`
```{r}
hello <- c(2, 4, 6) - c(1, 0, 1)
```

1.6) `c(2, 4, 6) * c(1, 0, 1)`
```{r}
goodbye <- c(2, 4, 6) * c(1, 0, 1)
```

1.7) `c(2, 4, 6) / c(1, 2, 1)`
```{r}
ciao <- c(2, 4, 6) / c(1, 2, 1)
```

However, things get *really* wild when we use vectors that do not have the same length.

Can you see what happened here?
it repeats this vector and it acts like its 1, 10, 1, 10
this is something that is really important 
whenever one vector is a multiple of the other, R will do this 
...
there is an error message, it starts again from the start
...
how many hot dogs did each contestant eat in 10 min
...
# tip: use alt and cursor to mark more than one line
```
...
```{r}
hotdog_2018
head(hotdog_2018) # the first 6 numbers in the data set
tail(hotdog_2018) # the last 6 numbers in the data set
```
...
```{r}
head(hotdog_names, 3) # it controls how many elements you want to see 
head(hotdog_names, 1)
...
```{r}
length(hotdog_names)
head(hotdog_names, 100) # when it is more elements than the vector has, nothing happens
head(100, hotdog_names) # use head() with the arguments in the wrong order
...
hotdog_names[c(3, 5)] # use a c function here, put a vector in here 
...
```{r}
hotdog_names[1:3] # in this case we do not need a c in front of 1:3
hotdog_names[c(1:3)]
...
```{r}
hotdog_names[4-3] # solution 1, so I get only the first name
```

```{r}
hotdog_names[min(1:10)] # minimum of one and ten is one, so the first name in the data set 
```
...
```{r}
tail(hotdog_names, 1) # Steve Hendry 
```
...
hotdog_names[length(hotdog_names)] # we do not need to specify max or min. bc length = 11  
...
```{r}
max(hotdog_2019) # maximum numbers of hotdogs eaten is 71

hotdog_2019 == max(hotdog_2019) # where is the 71?? 
hotdog_2019
...
hotdog_2019[index] # this corresponds to the number and the vector

hotdog_names[index] # we get the name of the element where this number is true 
# equals to:
hotdog_names[hotdog_2019 == max(hotdog_2019)] ## this is how I can combine functions and comparisons 
```
...
numbers[index] # it is like a filter function, when it hits true it gets shown, when it is false, it does not get shown 
```
...
index <- numbers < 3
# apply it to numbers
numbers[index] # only 1 and 2 will be shown bc 3 and 4 are false 
```
...
numbers[numbers < 3] # we do not need to save the index variable, this is the alternative form
```
...
number[<3] # this does not work, because R is just looking at the [ ].. and 
# there it is not defined what is supposed to be <3, we have to be explicit
```
...
(1:10)[1:10 > 6] # without the ( ) it does not work
(1:10)[2] # it does not work, bc this square bracket only refers to "10" and because 10 does not have a second element
...
female_names <- hotdog_names[hotdog_genders == "female"] # participants, where the gender is female
female_names

female_results <- hotdog_2019[hotdog_genders == "female"] # results of 2019, where the gender is female
female_results

female_names[female_results == max(female_results)] # der Name der Person, die weiblich ist und die meisten hotdogs gegessen hat
...
mean(c(1, 20, NA, 12, 3)) # there is a missing value... I do not know what the value is, so I show you NA
```
...
mean(c(1, 20, NA, 12, 3), na.rm = TRUE) # divides it only by 4 numbers, same as
mean(c(1, 20, 12, 3))
```
...
5.4) `table()`
```{r}
table(c(1, 20, NA, 12, 3), useNA = "ifany")
table(c(1, 20, NA, 12, 3), useNA = "no")
?table
```
...
```{r}
TRUE & NA  # it doesnt know what it is, so it does not know and says NA
FALSE & NA # no matter what this NA is... it would always be false
TRUE | NA # bc true is here, it will be true, no matter what the other  element says
FALSE | NA # false or true, here it does not know the answer
```
...
a_vector[a_vector == 12] # show me the elements of this vector that are 12, NA is also there bc it could be 12 
```
...
a_vector == NA # this is such a common mistake, you cannot use the logical comparison 
```
...
a_vector[!is.na(a_vector)] # ! == is not, gib mir alle Elemente, die nicht missing sind
4 != 4
```

...
```{r}
nerd <- read.csv("./data_sets/nerd_data_short.csv") # relative path, it works relative to the R markdown file, "." indicates where I am? 
```
...
head(age, 10)
head(nerd$age, 10) # you access a column with this
```
...
typeof(nerd) # why would it be a list? a data frame is a special type of list
```
...
is.data.frame(nerd) # TRUE = a collection of vectors
is.data.frame(age) # each column is a vector 
```
...
nerd[["age"]]
head(nerd[["age"]]) # full proof version, it will warn us when we misspell something
```
...
nerd[ ,1] # I want the fist column and all rows
nerd[1, ] # I want the first row and all columns 
```
...
nerd[ ,3] # I want the third column and all rows, before the comma is the row, behind columns
head(nerd[ , 3], 10) # give me the first 10 elements of the 3rd column 
```
...
```{r}
nerd[10, ] # rows go before the comma, columns go after the comma
row10 <- nerd[10, ]
row10
```
...
```{r}
nerd[10:12, ] # you select row 10 - 12 and all the columns
nerd[c(10:12), 1:ncol(nerd)]

```

1.4) Extract the value in the 70th row and the 10th column.
```{r}
nerd[70, 10]
nerd [10, 70]

names(nerd)[70] # what is the variable name of the 70th column? 
```
...
1.6) Use the "mess around data frame" to overwrite the entire first column. We will solve this task together with the course, but first think about the following questions: How would you, in principle, overwrite the entire first column? What would you need to replace it with?
```{r}
mess_around[,1]
head(mess_around)
mess_around[,1] <- rep("cake", 1000) # now the entire column is cake, "what is the new number or character", how many times 
mess_around[1,] <- rep("hot", 80)
```

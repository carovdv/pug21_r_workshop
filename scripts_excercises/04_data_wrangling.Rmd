...
if (!suppressWarnings(require(tidyverse, quietly=TRUE))) 
...
library(tidyverse)
search()
stats::filter() # be explicit what function from what package you want to use 
```
...
round(mean(example_vector), 2) # compute the mean and round it by 2 decimal places
```
...
example_vector %>% 
  mean() %>% 
  round(2) # production line, pipe the result in the next function
# reading from left to right, take the variable and put the mean in it and then take the mean and round it 
# use %>%  as a shortcut
```
...
vector_with_NAs %>% 
  ...
  head(., 4) %>% 
  mean(., na.rm = TRUE) %>% 
  as.character(.) # include the () so it is clear that these are functions
# we can identify arguments and functions by its position 
# there is an argument () that we cannot see it/it is hidden... it is always the data here
# we can insert the . so we are explicit... look there is data there
# hit return after every pipe
# use strg+i if you want to autocorrect the position of the code 
``` 
...
# attention: R will take everything that is in between Q1 and Q26 
# what is tidy about tidyverse is that I do not have to repeat the dataset before every variable

# alternative approach: use function starts_with
nerd %>%
  select (age, gender, married, starts_with("Q"))

# the same function in base R:
...
nerd %>% 
  select(voted, age, everything()) # everything = the rest of the data frame
# it is not changing the actual data frame nerd
# nothing is saved unless you save it as a variable or overwrite it
...
nerd_red <- 
  nerd %>% 
  select(age, gender, married, voted, nerdy, starts_with("Q")) # I can easily check if it is correct by commenting out the new data frame name
```
...
# filter for women below the age of 30 - use ,
nerd %>% 
  filter(gender == 2, age < 30) # the , is equal to &
```
...
```{r}
nerd_red %>% 
  filter(age == 22 | age == 26) # 74 participants

# alternative approach:
nerd_red %>% 
  filter(age %in% c(22, 26)) # is this "age 22 - 26" a part of this set? and if yes, how many? 
...
```{r}
nerd_red %>% 
  filter(gender == 1 | age > 30) # 452 participants 
```
...
nerd_red %>% 
  filter(gender == 3 & married %in% c(2, 3))
```
...
nerd_red %>% 
  ...
  count(.)
# counting, how many rows I have for each combination of gender and nerdy
...
nerd_red %>% 
  ...
  count(., name = "number_of_people")
# there are 285 men that were never married
# there are 51 men that are currently maried

nerd_table
ungroup(nerd_table) # remember to ungroup after you grouped 
```
...
nerd_red %>% 
  group_by(married) %>% 
  count (.)
# marital status is supposed to be 1 - 3, n = 5 with 0 

summary(nerd_red$age) # maximum age of 38822.00 years 
```
...
nerd_red %>% 
  filter(married %in% 1:3) # 995 people, - 5 people without marriage = 0 
...
nrow(nerd) - nerd_red %>% 
  count(.)
# difference between a tiblr and a data frame
# in a tiblr.. only the first 6 rows will be printed 
# maybe a tiblr cannot be used in the same 
...
nerd_young <-
nerd_red %>% 
  filter(age < 20) # speichere ein neues Datenset, in dem nur die Personen sind, die < 20 Jahre alt sind

age_young <-nerd_young$age # speichere eine neue Variable, in der nur das Alter ist
age_young

# really umstaendlicher weg... creating a new data set 
```
...
nerd_red %>% 
  filter(age < 20) %>% 
  pull(age)
# use pull if you want to filter a single variable
```
...
nerd_red %>% 
  filter(age < 20) %>% 
  select(age)
# we only selected the age column, but the output looks different 
# pull... we get the data as a single vector 
# select... we get a table with one column/a data frame
```
...
nerd_red %>% 
  arrange(gender, age)
# it is first sorting by gender and within gender group it is sorting by age 
```

4.2) Arrange `nerd_red` by `voted` and `age`. Let `voted` be ascending and `age` descending.
```{r}
nerd_red %>% 
  arrange(voted, desc(age)) # the default setting is ascending order and only if you do not want it you use desc
?arrange
```
...
nerd_red %>% 
  mutate(birthyear = 2018 - age, .before = gender) # mutate(name of the new column), by default it is sorted at the end of the data frame
?mutate
```
...
nerd_red %>% 
  mutate(age_character = as.character(age), .after = gender) 
```
```{r}
# how can I arrange the columns in my data frame? 
nerd_red %>% 
  select(married, gender, age, everything())
```
...
# modifying a column works the same way as creating a new column: mutate is modyfying existing one and adding new columns

nerd_red %>% 
  mutate(
    age2 = age - 1000, .before = gender,
    age = age + 1000
  )
```
...
# nerd_red is not overwritten, it does not affect the data frame unless I overwrite it
# we are overwritting this existing column, but only in the output 
```
...
# deselect a column with the function select

# alternative approach
nerd_red %>% 
  select(!age_character)

# if you want to deselect more columns
nerd_red %>% 
